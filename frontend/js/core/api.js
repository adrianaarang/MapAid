...
export async function listarEscenas(token) {
  return parsear(
    await fetch(`${BASE_URL}/deteccion/escenas`, { headers: cabeceras(token) }),
  );
}

export async function analizarEscena(escena, token) {
  return parsear(
    await fetch(`${BASE_URL}/deteccion/analizar`, {
      method: "POST",
...
export async function listarSugerencias({ escena, estado } = {}, token) {
  ...
  return parsear(
    await fetch(`${BASE_URL}/deteccion/sugerencias?${p}`, {
      headers: cabeceras(token),
    }),
  );
}
...
export async function obtenerResumen(escena, token) {
  ...
  if (escena) p.set("escena", escena);
  return parsear(
    await fetch(`${BASE_URL}/deteccion/resumen?${p}`, {
      headers: cabeceras(token),
    }),
  );
}
...
export async function validarSugerencia(id, decision, token) {
  return parsear(
    await fetch(`${BASE_URL}/validacion/${id}`, {
      method: "PATCH",
      headers: cabeceras(token),
...
export async function contarPorOrigen(escena, token) {
  ...
  if (escena) p.set("escena", escena);
  return parsear(
    await fetch(`${BASE_URL}/reportes/origen?${p}`, {
      headers: cabeceras(token),
    }),
  );
}
...
export async function usarActivacion(payload, token) {
  return parsear(
    await fetch(`${BASE_URL}/copernicus/usar-activacion`, {
      method: "POST",
      headers: cabeceras(token),
...
export async function listarActivaciones(token, max = 10) {
  return parsear(
    await fetch(`${BASE_URL}/copernicus/activaciones?max_resultados=${max}`, {
      headers: cabeceras(token),
    }),
  );
}
...
export function urlImagen(escena, momento) {
  return `../../data/raw/xbd/hold/images/${escena}_${momento}_disaster.png`;
}
