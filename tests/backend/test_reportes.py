...
def test_los_reportes_locales_entran_en_la_cola_de_revision():
    ...
    # Los reportes de personas no van a la cola de la IA, pero sí están
    # pendientes y la administradora los ve en su panel.
    sugerencias_pendientes = listar_reportes()
...
def test_se_puede_validar_un_reporte_local():
    """La administradora puede confirmar o rechazar reportes de personas."""

    from modules.validacion.schemas import ValidationAction, ValidationRequest
    from modules.validacion.services import validar

    reporte = crear_reporte(peticion())
    resultado = validar(reporte["id"], ValidationRequest(accion=ValidationAction.CONFIRM))
